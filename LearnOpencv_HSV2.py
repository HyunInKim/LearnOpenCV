...
while(True):
    ...
    ##########
    ret, img_binary = cv.threshold(img_result,51,255,cv.THRESH_BINARY)
    cv.imshow('frame',img_binary)
    #img_result2 = cv.bitwise_and(img_color,img_color,mask=img_binary)
    #cv.imshow('Result',img_result)
    ##########


    numOfLabels,img_lable,stats,centroids =cv.connectedComponentsWithStats(img_mask)
